...
/* ===== Estado inicial robusto: aunque GitHub borre hidden del HTML ===== */
function forceInitialState(){
  ...
  if($('view-incidence')) $('view-incidence').hidden=false;
  if($('view-report')) $('view-report').hidden=true;
  if($('view-review')) $('view-review').hidden=true;
  if($('view-learn')) $('view-learn').hidden=true;
  if($('view-progress')) $('view-progress').hidden=true;
  if($('tipsView')) $('tipsView').hidden=true;
  if($('quizzesView')) $('quizzesView').hidden=true;
}
forceInitialState();
...
async function setNewPassword(){
  ...
  $('resetMsg').textContent='✅ ¡Contraseña actualizada! Ya podés entrar con tu nueva contraseña.';
  RECOVERING=false;
  await db.auth.signOut();
  setTimeout(()=>{
    $('resetScreen').hidden=true;
    $('authScreen').hidden=false;
    showAuthMsg('Iniciá sesión con tu nueva contraseña.');
  },1500);
}
...
(async()=>{
  ...
  if(data.session&&!RECOVERING)startApp(data.session.user);
})();
...
function updatePointsUI(){
  $('pointsCounter').textContent=(PROFILE.points||0)+' pts';
}

async function addPoints(pts){
  const np=(PROFILE.points||0)+pts;
  const lvl=LEVELS.filter(l=>np>=l.min).length;
...
async function loadActions(){
  ...
  ACTIONS=data||[];
  const box=$('actionsGrid');
  if(!ACTIONS.length){
    box.innerHTML='<div class="card" style="grid-column:1 / -1">No hay acciones cargadas todavía.</div>';
    return;
  }
  box.innerHTML=ACTIONS.map(a=>'<div class="action-card '+(a.id==='house_tour'?'star':'')+'" onclick="startReport(\''+a.id+'\')">'+(a.id==='house_tour'?'<span class="star-tag">★ destacada</span>':'')+'<div class="action-ico">'+a.icon+'</div><h4>'+a.title+'</h4><span class="badge">'+a.location+'</span><span class="pts">+'+a.points+' pts</span></div>').join('');
}

function startReport(id){
  reportState={action:ACTIONS.find(x=>x.id===id),file:null};
  renderStep1();
  openModal();
}
function renderStep1(){
  const a=reportState.action;
  setModal('<div class="step-ind">Paso 1 de 3</div><div class="step-ico">'+a.icon+'</div><h2 style="text-align:center">'+a.title+'</h2><p style="text-align:center;margin:8px 0">'+a.description+'</p><div style="text-align:center;margin:10px 0"><span class="badge">'+a.location+'</span><span class="badge">'+a.kind+'</span><span class="pts">+'+a.points+' pts</span></div><p class="small" style="text-align:center">Vas a completar esta acción y sacar una foto que lo demuestre.</p><button class="btn-primary" onclick="renderStep2()">Entendido, continuar</button>');
}
function renderStep2(){
  setModal('<div class="step-ind">Paso 2 de 3</div><h2>Sacá la foto</h2><p class="small">Sacá una foto que muestre la acción completada.</p><input type="file" id="reportPhoto" accept="image/*" capture="environment" onchange="onPhoto(event)"><div id="thumbBox"></div><button class="btn-outline" onclick="renderStep1()">← Volver</button><button class="btn-primary" id="toStep3" onclick="renderStep3()" disabled style="opacity:.5">Continuar</button>');
}
function onPhoto(e){
  const f=e.target.files[0];
  if(!f)return;
  reportState.file=f;
  $('thumbBox').innerHTML='<img src="'+URL.createObjectURL(f)+'" class="thumb"><div class="small">✔ Foto lista</div>';
  const b=$('toStep3');
  b.disabled=false;
  b.style.opacity=1;
}
function renderStep3(){
  const a=reportState.action,u=reportState.file?URL.createObjectURL(reportState.file):'';
  setModal('<div class="step-ind">Paso 3 de 3</div><h2>Contexto y enviar</h2>'+(u?'<img src="'+u+'" class="thumb">':'')+'<label class="small">Agregá contexto (opcional)</label><textarea id="reportNote" placeholder="Contá algo sobre este reporte (opcional). Ej: Di vuelta 3 baldes del patio"></textarea><button class="btn-outline" onclick="renderStep2()">← Volver</button><button class="btn-primary" onclick="submitReport()">Enviar reporte (+'+a.points+' pts)</button><div id="reportMsg" class="msg"></div>');
}
async function submitReport(){
  const a=reportState.action,note=$('reportNote').value.trim(),file=reportState.file,msg=$('reportMsg');
  msg.textContent='Subiendo...';
  try{
    let photoUrl=null;
    if(file){
      const path=CURRENT_USER.id+'/'+Date.now()+'_'+file.name;
      const{error:e}=await db.storage.from('reports').upload(path,file);
      if(e)throw e;
...
async function loadReview(){
  ...
  if((PROFILE.points||0)<REVIEWER_MIN){
    box.innerHTML='<div class="card locked"><div class="big">🔒</div><h3>Rol Revisor bloqueado</h3><p>Alcanzá el nivel <b>🛡️ Guardián del Barrio</b> (350 pts) para desbloquear el rol de Revisor y ganar puntos ayudando a validar reportes de tu comunidad.</p></div>';
    return;
  }
...
function openReviewModal(r){
  setModal('<div class="modal-photo">'+(r.photo_url?'<img src="'+r.photo_url+'" style="width:100%;height:100%;object-fit:cover;border-radius:12px">':'📷')+'</div><h2>'+((r.action_types&&r.action_types.title)||r.action_type)+'</h2><div class="small">👤 '+(r.city||'-')+'</div>'+(r.note?'<div class="note-box">📝 '+r.note+'</div>':'<div class="small" style="margin:10px 0">Sin nota de contexto.</div>')+'<div class="vote-row"><button class="v-ok" onclick="voteReport('+r.id+',\'correcto\')">✅ Correcto</button><button class="v-doubt" onclick="voteReport('+r.id+',\'dudoso\')">❓ Dudoso</button><button class="v-no" onclick="voteReport('+r.id+',\'no_corresponde\')">⚠️ No corresponde</button></div>');
  openModal();
}
async function voteReport(id,vote){
  const{error}=await db.from('report_reviews').insert({report_id:id,reviewer_id:CURRENT_USER.id,vote});
  if(error){alert('Ya revisaste este reporte o hubo un error.');return;}
  await addPoints(3);closeModal();loadReview();
}
...
async function refreshLearnCounters(){
  ...
  const{count:q}=await db.from('quiz_answers').select('*',{count:'exact',head:true}).eq('user_id',CURRENT_USER.id).eq('is_correct',true);
  $('tipsCount').textContent=(t||0)+' leídos';$('quizCount').textContent=(q||0)+' resueltos';
}
...
async function toggleTip(id){
  const el=$('tip-'+id);el.classList.toggle('open');
  if(el.classList.contains('open')&&!el.classList.contains('read')){
    ...
    if(!error){el.classList.add('read');await addPoints(2);refreshLearnCounters();bumpDaily();}
  }
}
